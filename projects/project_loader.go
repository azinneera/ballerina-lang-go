...
type ProjectLoadConfig struct {
	...
	// If nil, defaults to <projectFs>/.ballerina/ for the local cache.
...
//  3. Otherwise, default to <projectFs>/.ballerina/ for the local cache
func (l *ProjectLoader) createEnvironmentWithRepositories(cfg ProjectLoadConfig) *Environment {
	repos := cfg.Repositories

	// If no repositories provided, create default repositories
	if len(repos) == 0 {
		homeFs := l.ballerinaHomeFs
		if homeFs == nil {
			// Default to project-local .ballerina directory
