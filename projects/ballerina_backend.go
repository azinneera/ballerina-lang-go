...
// NewBallerinaBackend creates a BallerinaBackend from a PackageCompilation. BIR is normally
// already generated by this point — Compilation pipelines it into Phase 2 whenever
// CompilationOptions.GenerateCode() is set (the default) — so this typically just wraps
// already-compiled state. performCodeGen backfills BIR for any module that doesn't have it
// yet (e.g. because the caller set GenerateCode(false), as a language server would).
...
// performCodeGen generates BIR for any module that doesn't already have it, in topological
// order. generateCodeInternal is idempotent, so this is a no-op for modules Compilation
// already generated BIR for.
