...
public isolated function createBallerinaError(string errorMessage, string category) returns TestError {
    // category is unused in jballerina too; discarded to satisfy the unused-param check.
    _ = category;
...
// byteSubstring stands in for the not-yet-implemented lang.string:substring (see TODO.md).
// Byte-based, not Unicode-codepoint-safe.
isolated function byteSubstring(string str, int startIndex, int endIndex) returns string {
    byte[] bytes = str.toBytes();
    byte[] sliced = [];
    foreach int i in startIndex ..< endIndex {
        sliced.push(bytes[i]);
    }
    string|error result = string:fromBytes(sliced);
    if result is error {
        return str;
    }
    return result;
...
// mapHasKey stands in for the not-yet-implemented lang.map:hasKey (see TODO.md).
isolated function mapHasKey(map<anydata> m, string k) returns boolean {
...
isolated function getKeyArray(map<anydata> valueMap) returns string[] {
    string[] keyArray = valueMap.keys();
    foreach string keyVal in keyArray {
        anydata value = valueMap[keyVal];
...
isolated function getMapValueDiff(map<anydata> actualMap, map<anydata> expectedMap) returns string {
    string diffValue = "";
    ...
    if keyDiff != "" {
        diffValue = diffValue + keyDiff + "\n" + valueDiff;
    } else {
        diffValue = diffValue + valueDiff;
    }
    return diffValue;
}

// ValueComparisonResult replaces jballerina's `[string, int]` tuple return: list-binding-pattern
// assignments (`[a, b] = ...`) aren't implemented yet (see TODO.md).
type ValueComparisonResult record {|
    string diff;
    int count;
|};

isolated function getValueComparison(anydata actual, anydata expected, string keyVal, int count)
        returns ValueComparisonResult {
    int diffCount = count;
    string diff = "";
    string expectedType = getBallerinaType(expected);
    string actualType = getBallerinaType(actual);
    if expectedType != actualType {
        diff = diff + string `${"\n"}key: ${keyVal}${"\n \n"}`
                + string `expected value${"\t"}: <${expectedType}> ${sprintf("%s", expected)}`
                + string `${"\n"}actual value${"\t"}: <${actualType}> ${sprintf("%s", actual)}`;
        diffCount = diffCount + 1;
    } else if actual is map<anydata> && expected is map<anydata> {
        int orderCount = diffCount;
        foreach string childKey in actual.keys() {
            anydata actualChildVal = actual[childKey];
            if mapHasKey(expected, childKey) {
                anydata expectedChildVal = expected[childKey];
                if expectedChildVal != actualChildVal {
                    ValueComparisonResult childResult = getValueComparison(actualChildVal, expectedChildVal,
                            keyVal + "." + childKey, diffCount);
                    diffCount = childResult.count;
                    if diffCount != (orderCount + 1) {
                        diff = diff + "\n";
                    }
                    diff = diff + childResult.diff;
                }
            }
        }
    } else {
        diff = diff + string `${"\n"}key: ${keyVal}${"\n \n"}expected value${"\t"}: ${sprintf("%s", expected)}`
                + string `${"\n"}actual value${"\t"}: ${sprintf("%s", actual)}`;
        diffCount = diffCount + 1;
    }
    return {diff: diff, count: diffCount};
}

isolated function compareMapValues(map<anydata> actualMap, map<anydata> expectedMap) returns string {
    string diff = "";
    string[] actualKeyArray = actualMap.keys();
    int count = 0;
    foreach string keyVal in actualKeyArray {
        if mapHasKey(expectedMap, keyVal) {
            anydata expected = expectedMap[keyVal];
            anydata actual = actualMap[keyVal];
            if expected != actual {
                ValueComparisonResult result = getValueComparison(actual, expected, keyVal, count);
                count = result.count;
                if count != 1 {
                    diff = diff + "\n";
                }
