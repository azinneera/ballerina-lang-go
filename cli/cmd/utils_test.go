...
// TestOutputStyleFor covers both branches of outputStyleFor: every CLI
...
func TestSeverityColor(t *testing.T) {
	...
	if got := s.severityColor(diagnostics.Warning); got != s.yellow {
		t.Errorf("severityColor(Warning) = %q, want %q", got, s.yellow)
	}
	if got := s.severityColor(diagnostics.Error); got != s.red {
		t.Errorf("severityColor(Error) = %q, want %q", got, s.red)
	}
}
