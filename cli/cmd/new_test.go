...
func TestNewPackage_WithTemplate(t *testing.T) {
	...
	testCases := []struct {
		...
		{"default", "main.bal", "public function main()", "main_test.bal", "@test:Config {}"},
		{"main", "main.bal", "public function main()", "main_test.bal", "@test:Config {}"},
		{"service", "service.bal", "service / on new http:Listener", "service_test.bal", "@test:Config {}"},
		{"lib", "lib.bal", "public function hello(string? name) returns string", "lib_test.bal", "test:assertEquals"},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.template, func(t *testing.T) {
			...
			// Verify the matching sample test file exists under tests/
			testPath := filepath.Join(pkgPath, "tests", tc.testFile)
			testContent, err := os.ReadFile(testPath)
			if err != nil {
				t.Fatalf("expected test file %s to exist: %v", testPath, err)
			}
			if !strings.Contains(string(testContent), tc.testContains) {
				t.Errorf("test file %s missing expected content '%s':\n%s",
					tc.testFile, tc.testContains, testContent)
			}
			if !strings.Contains(string(testContent), "import ballerina/test;") {
				t.Errorf("test file %s missing 'import ballerina/test;':\n%s", tc.testFile, testContent)
			}

			// Verify other template test files do NOT exist
			otherTests := []string{"main_test.bal", "lib_test.bal", "service_test.bal"}
			for _, other := range otherTests {
				if other == tc.testFile {
					continue
				}
				if _, err := os.Stat(filepath.Join(pkgPath, "tests", other)); err == nil {
					t.Errorf("unexpected test file %s exists for template %s", other, tc.template)
				}
			}
		})
	}
...
func assertPackageStructure(t *testing.T, projectPath string) {
	...
	// Verify the sample test file exists (default template)
	testPath := filepath.Join(projectPath, "tests", "main_test.bal")
	if _, err := os.Stat(testPath); os.IsNotExist(err) {
		t.Errorf("tests/main_test.bal does not exist: %s", testPath)
	}
