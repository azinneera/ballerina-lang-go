...
// getSourceName extracts a display name from the file path.
func getSourceName(fileName string) string {
...
func runBallerina(cmd *cobra.Command, args []string) error {
	...
	if flags != 0 {
		...
		if runOpts.logFile != "" {
			...
			if err != nil {
				printError(fmt.Errorf("error creating log file %s: %w", runOpts.logFile, err), "", false)
	...
	if runOpts.dumpAST || runOpts.dumpBIR {
		...
		if runOpts.dumpBIR {
			...
			// Print BIR with separators
			fmt.Fprintln(os.Stderr)
			fmt.Fprintln(os.Stderr, "==================BIR==================")
			fmt.Fprintln(os.Stderr, "=======================================")
	...
	// TODO: Actual execution will be implemented here
